...
class ModuleFeature {
  ...
  async performComplexOperation() {
    return this.withErrorReporting(
      () => this.doComplexWork(),
...
const attribution = debug.ErrorAttribution.attributeToModule(
...
Hooks.on('updateToken', boundary.wrapHook('updateToken', (tokenDocument, change, options, userId) => {
...
async function updateTokenSafely(tokenId, updates) {
  return boundary.wrapAsync(async () => {
